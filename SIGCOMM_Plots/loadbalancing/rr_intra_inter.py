...
df.SDResource.plot(kind='bar', color='royalblue', ax=ax,error_kw=dict(elinewidth=2,ecolor='k'), linewidth=2, width=width, capsize=10, hatch='.', position=1+0.1, fontsize='60')
df.SLOViolation.plot(kind='bar', color='salmon', ax=ax2, error_kw=dict(elinewidth=2,ecolor='k'), linewidth=2, width=width, capsize=10, hatch ='/', position=0,  fontsize='60')
...
ax.set_ylabel('Std. Dev of Load Dist. (%)', fontsize='60')
ax2.set_ylabel('SLO Violations (%)', fontsize='60')
...
plt.legend(handles=[mar, gre], loc='upper right', fontsize=60, borderpad=None, borderaxespad=None,fancybox=True, framealpha=0.5)
